...
func OpenDefault(dbPath string) (*DB, error) {
	o := &opt.Options{
		...
		WriteBuffer:            1024 * 1024 * 8,
		BlockCacheCapacity:     1024 * 1024 * 16,
...
func makeHashKey(name, key []byte) []byte {
	return Bconcat(HashKeyPrefix, name, SplitChar, key)
}

func makeHashKeyPrefix(name string) []byte {
	return Bconcat(HashKeyPrefix, []byte(name), SplitChar)
}
...
// BytesToUint64  return an uint64 of v
